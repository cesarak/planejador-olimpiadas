...
import { PersistOptionsInput, SchedulingInput, SchedulingResult } from "./types";

interface PersistPayload {
  input: SchedulingInput;
  result: SchedulingResult;
  options: PersistOptionsInput;
}

export const persistScheduleVersion = async ({
  input,
  result,
  options,
}: PersistPayload): Promise<{
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const matchIdBySource = new Map<string, string>();
...
export const listScheduleVersions = async (
  tenantId: string,
  ...
  const where = {
    ...
    ...(options.nomeContains ? { name: { contains: options.nomeContains, mode: "insensitive" as const } } : {}),
  };

  const [total, versions] = await Promise.all([
    prisma.scheduleVersion.count({ where }),
...
export const getScheduleVersionById = async (versionId: string, tenantId?: string) => {
...
const allocationSignature = (item: {
  ...
  `${item.match.sourceMatchId}|${item.status}|${item.day ?? ""}|${item.startMin ?? ""}|${item.endMin ?? ""}|${
    item.localId ?? ""
  }`;

export const compareScheduleVersions = async (
  versionAId: string,
  versionBId: string,
  tenantId?: string,
) => {
  ...
  const allMatchIds = new Set<string>([...mapA.keys(), ...mapB.keys()]);
  const mudancasDetalhadas: Array<{
    sourceMatchId: string;
    tipo: "status" | "horario" | "quadra" | "motivo" | "novo_na_b" | "removido_na_b";
    versaoA: {
      status: string | null;
      day: string | null;
      startMin: number | null;
  ...
  return {
    ...
    resumo_diff: {
      ...
      total_confrontos_com_alguma_mudanca:
        mudaramStatus +
        mudaramHorario +
        mudaramQuadra +
        mudaramMotivo +
