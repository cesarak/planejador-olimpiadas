...
const createCompetitionSchema = z.object({
  ...
  stepGrid: z.number().int().min(5).max(60),
...
const updateCompetitionSchema = z
  .object({
    ...
    stepGrid: z.number().int().min(5).max(60).optional(),
    days: z.array(z.string().regex(/^\d{4}-\d{2}-\d{2}$/)).min(1).optional(),
  })
  .refine(
    (payload) =>
...
const ensureTimeWindow = (startMin: number, endMin: number): string | null => {
  if (endMin <= startMin) {
...
export const registerCompetitionRoutes = async (app: FastifyInstance): Promise<void> => {
  ...
  app.get("/api/competitions", async (request, reply) => {
    ...
    const accessContext = await resolveAccessContext(
      ...
      parsedQuery.success ? parsedQuery.data.tenantId : undefined,
    );

    if (!accessContext) {
      return reply.status(400).send({
        message:
          "Contexto invalido para listar campeonatos. Informe tenantId ou token de autenticacao valido.",
  ...
  app.post("/api/competitions", async (request, reply) => {
    ...
    if (!accessContext) {
      return reply.status(400).send({
        message:
          "Contexto invalido para criar campeonato. Informe tenantId ou token de autenticacao valido.",
      });
    }
  ...
  app.put("/api/competitions/:competitionId", async (request, reply) => {
    ...
    const accessContext = await resolveAccessContext(
      request.headers.authorization,
      parsedBody.success ? parsedBody.data.tenantId : undefined,
    );
